...
function Login(props) {
        ...
        if (name && password) {
            userService.login(name, password)
                .then((success) => {
                    ...
                    dispatch(login(user))
                    setTimeout(() => { dispatch(logout()); props.history.push('/logout') }, 20 * 60 * 1000);
    ...
    return (
        ...
        <ReactTypingEffect speed={100} eraseDelay={100} className="typingeffect" text={['Stocks',
                            'Mutual Funds', 'Fixed Deposits', 'Gold']} />
                    </div>
